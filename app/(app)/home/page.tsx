...
export default async function HomePage() {
  ...
  // Load streak, today's session, rasa, recent scores in parallel
  ...
  return (
    <div className="flex-1 flex flex-col px-5 py-7 overflow-y-auto">

      {/* Header */}
      <div className="flex items-center justify-between mb-7">
        <div>
          <p className="text-stone-400 text-sm">{timeGreeting()},</p>
          <h1 className="text-2xl font-semibold text-stone-900 capitalize">{displayName}</h1>
        </div>
      ...
      {/* Streak card */}
      <div className="bg-teal-700 rounded-2xl p-5 mb-5 text-white">
        ...
        <p className="text-teal-200 text-sm">{streakMessage(currentStreak)}</p>
      </div>

      {/* Rasa of the week */}
      {rasa && (
        <div className="rounded-2xl p-4 mb-5 border border-stone-100 bg-white">
          <p className="text-xs text-stone-400 uppercase tracking-wider mb-1">This week&apos;s rasa</p>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <span className="text-stone-400 text-sm ml-2">{rasa.theme}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mb-5">
        ...
        {sessionDoneToday ? (
          ...
          <Link href="/practice" className="block bg-white rounded-2xl p-5 border border-stone-100 hover:border-teal-200 transition-colors">
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div
                      ...
                      style={{ height: h, backgroundColor: (s.brain_shift_score ?? 50) >= 60 ? "#0F6E56" : "#d6d3d1" }}
                    />
                  </div>
                );
              })}
