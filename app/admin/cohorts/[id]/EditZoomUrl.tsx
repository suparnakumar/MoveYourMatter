...
export default function EditZoomUrl({ cohortId, current }: { cohortId: string; current: string | null }) {
  ...
  if (editing) {
    return (
      <div className="bg-white rounded-2xl border border-stone-200 px-5 py-4">
        <p className="text-xs text-stone-400 mb-2">Zoom room URL</p>
        <input
          type="url"
          value={value}
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 px-5 py-4">
      <p className="text-xs text-stone-400 mb-1">Zoom</p>
      <div className="flex items-center justify-between gap-2">
        {current ? (
          <a href={current} target="_blank" rel="noopener noreferrer"
            className="text-sm font-medium text-teal-700 hover:underline truncate">
            Join room ↗
          </a>
        ) : (
        ...
        <button
          ...
          className="text-xs text-stone-400 hover:text-teal-700 transition-colors flex-shrink-0"
        >
          Edit
        </button>
      </div>
