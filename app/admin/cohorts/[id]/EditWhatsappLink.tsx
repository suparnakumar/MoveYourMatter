...
export default function EditWhatsappLink({ cohortId, current }: { cohortId: string; current: string | null }) {
  ...
  async function handleSave() {
    ...
    const { error: dbError } = await supabase
      .from("cohorts")
      .update({ whatsapp_link: value || null })
      .eq("id", cohortId);
  ...
  if (editing) {
    return (
      <div className="bg-white rounded-2xl border border-stone-200 px-5 py-4">
        <p className="text-xs text-stone-400 mb-2">WhatsApp group link</p>
        <input
          type="url"
          value={value}
